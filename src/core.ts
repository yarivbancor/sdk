...
export class Core {
  ...
  async create(settings: Settings) {
    ...
    if (settings.eosNodeEndpoint) this.blockchains[BlockchainType.EOS] = await EOS.create(settings.eosNodeEndpoint);
  }
  ...
  async destroy(): Promise<void> {
    ...
    if (this.blockchains[BlockchainType.EOS]) await EOS.destroy(this.blockchains[BlockchainType.EOS] as EOS);
  }

  async refresh(): Promise<void> {
    for (let blockchainType in this.blockchains) await this.blockchains[blockchainType].refresh();
  }

  async getPaths(sourceToken: Token, targetToken: Token, amount: string = '1'): Promise<Token[][]> {
    ...
    // single blockchain path
    if (sourceBlockchain == targetBlockchain) {
      return await this.getPathsFunction(sourceToken.blockchainType, sourceToken, targetToken);
    }

    // cross blockchain path
    // get all possible paths between source to anchor
    const sourcePaths = await this.getPathsFunction(
      sourceToken.blockchainType,
      sourceToken,
      sourceBlockchain.getAnchorToken(),
    );
    // get all possible paths between anchor to target
    const targetPaths = await this.getPathsFunction(
      targetToken.blockchainType,
      targetBlockchain.getAnchorToken(),
      targetToken,
    );

    // create full paths from source to target
    const paths = [];
  ...
  async getPathAndRate(
    ...
    if (sourceBlockchain == targetBlockchain) {
      const paths = await this.getPathsFunction(sourceToken.blockchainType, sourceToken, targetToken);
  ...
  private async getPathsFunction(
    blockchainType: BlockchainType,
    sourceToken: Token,
    targetToken: Token,
  ): Promise<Token[][]> {
    // special case for source token == target token
    if (helpers.isTokenEqual(sourceToken, targetToken)) return [[sourceToken]];
  ...
  private async getRates(blockchainType: BlockchainType, paths: Token[][], amount: string = '1'): Promise<string[]> {
    ...
    if (paths.length == 1 && helpers.isTokenEqual(paths[0][0], paths[0][paths[0].length - 1])) return [amount];
