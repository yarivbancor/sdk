...
export class Ethereum implements Blockchain {
  ...
  static async create(nodeEndpoint: string | Object): Promise<Ethereum> {
    ...
    const contractRegistry = new ethereum.web3.eth.Contract(
      abis.ContractRegistry,
      getContractAddresses(ethereum).registry
    );
    const bancorNetworkAddress = await contractRegistry.methods
      .addressOf(Web3.utils.asciiToHex("BancorNetwork"))
      .call();
    const converterRegistryAddress = await contractRegistry.methods
      .addressOf(Web3.utils.asciiToHex("BancorConverterRegistry"))
      .call();
    ethereum.bancorNetwork = new ethereum.web3.eth.Contract(
      abis.BancorNetwork,
      bancorNetworkAddress
    );
    ethereum.converterRegistry = new ethereum.web3.eth.Contract(
      abis.BancorConverterRegistry,
      converterRegistryAddress
    );
    ethereum.multicallContract = new ethereum.web3.eth.Contract(
      abis.MulticallContract,
      getContractAddresses(ethereum).multicall
    );
    ethereum.decimals = {
      ...CONTRACT_ADDRESSES[ethereum.networkType].nonStandardTokenDecimals,
    };
  ...
  getAnchorToken(): Token {
    return {
      ...
      blockchainId: getContractAddresses(this).anchorToken,
    };
  }

  async getPaths(sourceToken: Token, targetToken: Token): Promise<Token[][]> {
    const sourceAddress = Web3.utils.toChecksumAddress(
      sourceToken.blockchainId
    );
    const targetAddress = Web3.utils.toChecksumAddress(
      targetToken.blockchainId
    );
    const addressPaths = this.getPathsFunc(sourceAddress, targetAddress);
    return addressPaths.map((addressPath) =>
      addressPath.map((address) => ({
        blockchainType: BlockchainType.Ethereum,
  ...
  async getRates(
    ...
    const addressPaths = tokenPaths.map((tokenPath) =>
      tokenPath.map((token) => Web3.utils.toChecksumAddress(token.blockchainId))
    );
    const sourceDecimals = await getDecimals(this, addressPaths[0][0]);
    const targetDecimals = await getDecimals(
      this,
      addressPaths[0].slice(-1)[0]
    );
    const tokenRates = await getRatesSafe(
      this,
      addressPaths,
      helpers.toWei(tokenAmount, sourceDecimals)
    );
    return tokenRates.map((tokenRate) =>
      helpers.fromWei(tokenRate, targetDecimals)
    );
  }

  async getConverterVersion(converter: Converter): Promise<string> {
    return (await converterVersion.get(this.web3, converter.blockchainId))
      .value;
  }
  ...
  async getConversionEvents(
    ...
    return await conversionEvents.get(
      this.web3,
      this.decimals,
      token.blockchainId,
      fromBlock,
  ...
  async getAnchors(): Promise<string[]> {
    return this.converterRegistry.methods.getAnchors().call();
  }

  async getConvertibleTokens(): Promise<string[]> {
    return this.converterRegistry.methods.getConvertibleTokens().call();
  }
  ...
  getSomePathsFunc(sourceToken: string, targetToken: string): string[][] {
    const commonTokens = this.graph[sourceToken].filter((token) =>
      this.graph[targetToken].includes(token)
    );
    const paths = commonTokens.map((commonToken) => [
      sourceToken,
      commonToken,
      targetToken,
    ]);
    ...
    for (const pivotToken1 of pivotTokens) {
      for (const pivotToken2 of pivotTokens) {
        const sourcePath = getOnePathRecursive(
          this.trees[pivotToken1],
          sourceToken
        );
        const middlePath = getOnePathRecursive(
          this.trees[pivotToken2],
          pivotToken1
        );
        const targetPath = getOnePathRecursive(
          this.trees[pivotToken2],
          targetToken
        );
        paths.push(
          getMergedPath(sourcePath.concat(middlePath.slice(1)), targetPath)
        );
      }
    }
    return Array.from(
      new Set<string>(paths.map((path) => path.join(",")))
    ).map((path) => path.split(","));
  }
...
export const getDecimals = async function(ethereum, token) {
  if (ethereum.decimals[token] == undefined) {
    const tokenContract = new ethereum.web3.eth.Contract(
      abis.ERC20Token,
      token
    );
...
async function getRatesSafe(ethereum, paths, amount) {
  try {
    ...
    if (paths.length > 1) {
      ...
      const arr2 = await getRatesSafe(
        ethereum,
        paths.slice(mid, paths.length),
        amount
      );
      return [...arr1, ...arr2];
    }
...
export const getRates = async function(ethereum, paths, amount) {
  ...
  const [
    blockNumber,
    returnData,
  ] = await ethereum.multicallContract.methods.aggregate(calls, false).call();
  return returnData.map((item) =>
    item.success ? Web3.utils.toBN(item.data.substr(0, 66)).toString() : "0"
  );
};

export const getTokens = async function(ethereum) {
  const convertibleTokens = await ethereum.converterRegistry.methods
    .getConvertibleTokens()
    .call();
  const calls = convertibleTokens.map((convertibleToken) => [
  ...
  const [
    blockNumber,
    returnData,
  ] = await ethereum.multicallContract.methods.aggregate(calls, true).call();
  const smartTokenLists = returnData.map((item) =>
    Array.from(Array((item.data.length - 130) / 64).keys()).map((n) =>
      Web3.utils.toChecksumAddress(item.data.substr(64 * n + 154, 40))
    )
  );
  return convertibleTokens.reduce(
    (obj, item, index) => ({ ...obj, [item]: smartTokenLists[index] }),
    {}
  );
};
...
function getGraph(tokens) {
  ...
  for (const convertibleToken in tokens) {
    for (const smartToken of tokens[convertibleToken]) {
      ...
      else smartTokens[smartToken].push(convertibleToken);
    }
  }
...
function updateGraph(graph, key, value) {
  if (graph[key] == undefined) graph[key] = [value];
...
function getAllPathsRecursive(paths, graph, tokens, destToken) {
  ...
  if (prevToken == destToken) paths.push(tokens);
  else
    for (const pair of graph[prevToken].filter(
      (pair) => !tokens.includes(pair[1])
    ))
