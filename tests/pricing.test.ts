...
const eosTable = {
  stat: {
    AAABBB: {
      aaabbbaaabbb: [{ supply: '400000.0000 AAACCC', max_supply: '250000000.0000 AAACCC', issuer: 'converteraab' }],
    },
    AAACCC: {
      aaacccaaaccc: [{ supply: '200000.0000 AAACCC', max_supply: '250000000.0000 AAACCC', issuer: 'converteraac' }],
    },
  },
  settings: {
  ...
  reserves: {
    converteraab: {
      converteraab: [
        { contract: 'aaaaaaaaaaaa', currency: '0.0 AAA', ratio: 500000, p_enabled: 1 },
        { contract: 'bbbbbbbbbbbb', currency: '0.0 BBB', ratio: 500000, p_enabled: 1 },
      ],
    },
    converteraac: {
      converteraac: [
        { contract: 'aaaaaaaaaaaa', currency: '0.0 AAA', ratio: 500000, p_enabled: 1 },
        { contract: 'cccccccccccc', currency: '0.0 CCC', ratio: 500000, p_enabled: 1 },
      ],
    },
  },
  accounts: {
...
describe('rates test', () => {
  ...
  it('getPathAndRate from ethereum token to ethereum token using getAllPathsFunc', async () => {
    ...
    const spyGetPathsFunc = jest.spyOn(blockchain, 'getPathsFunc').mockImplementationOnce(blockchain.getAllPathsFunc);
    ...
    const spyGetRates = jest
      ...
      .mockImplementationOnce(() => Promise.resolve(['21113', '224999733', '22425888633', '224258633', '2249733']))
      .mockImplementationOnce(() => Promise.resolve(['225888633', '22524999733', '225249733', '2258633', '213']));
  ...
  it('getPathAndRate from ethereum token to ethereum token using getSomePathsFunc', async () => {
    const blockchain = sdk._core.blockchains[BlockchainType.Ethereum] as ethereum.Ethereum;
    const spyGetPathsFunc = jest.spyOn(blockchain, 'getPathsFunc').mockImplementationOnce(blockchain.getSomePathsFunc);

    const spyGetContractAddresses = jest.spyOn(ethereum, 'getContractAddresses').mockImplementation(() => ({
    ...
    const spyGetRates = jest
      .spyOn(ethereum, 'getRates')
      .mockImplementationOnce(() => Promise.reject())
      .mockImplementationOnce(() => Promise.resolve(['2258633']))
      .mockImplementationOnce(() => Promise.resolve(['2249733']));
  ...
  it('getPathAndRate from ethereum token to eos token', async () => {
    ...
    jest.spyOn(sdk._core.blockchains[BlockchainType.EOS], 'getAnchorToken').mockImplementationOnce(() => ({
      blockchainType: BlockchainType.EOS,
      blockchainId: 'aaaaaaaaaaaa',
      symbol: 'AAA',
    }));
